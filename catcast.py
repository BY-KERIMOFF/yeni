...
def get_current_program(channel_id):
    """Send POST request to get current program information with proper headers."""
    url = f"https://api.catcast.tv/api/channels/{channel_id}/getcurrentprogram"
    
    # ⭐ ƏN VACİB HİSSƏ - REFERER BAŞLIĞI ⭐
    headers = {
...
def create_m3u8_file(slug, channel_id, token, output_dir="catcast"):
    """Create M3U8 playlist file with V2 URL format."""
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    
    # V2 URL formatında yarat (daha etibarlı)
    v2_url = f"https://v2.catcast.tv/content/{channel_id}/index.m3u8?token={token}"
    
    # M3U8 content - #EXTVLCOPT ile referer əlavə edirik
    m3u8_content = f"""#EXTM3U
#EXTVLCOPT:http-referer=https://catcast.tv/
#EXT-X-VERSION:3
...
def main(auto_update_interval=None):
    ...
    try:
        ...
        example_config = [
            {"id": 50544, "slug": "mrbean"}
        ]
        with open("catcast-config.json", 'w', encoding='utf-8') as f:
            json.dump(example_config, f, indent=2)
        print("✓ Created catcast-config.json with example channel")
        print("  Please edit it with your channels and run again!")
        return
    ...
    if auto_update_interval:
        ...
        # Single run mode
        print("🔧 Single run mode - processing all channels once\n")
        successful_channels = []
        failed_channels = []
...
if __name__ == "__main__":
    ...
    # 2. Avtomatik yeniləmə üçün (hər 5 dəqiqədə = 300 saniyə):
